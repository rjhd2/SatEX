...
for MIN_OR_MAX in MINMAX:
    for REGION in REGIONS:
        ...
        with open('climpact_indices_2x3_subfigures'+MIN_OR_MAX+'_'+REGION+'.txt', 'w') as f:
            for i in range(len(climpact_indices)):
                ICLIMP = climpact_indices[i]
                IGHCNDEX = ghcndex_climpact_indices[i]             
                ...
                f.write('\n\n\n')
                f.write('\\begin{figure}\n')
                f.write('\\includegraphics[trim=0 25 0 15,clip,width=0.49\\textwidth]{/scratch/vportge/plots/Climpact/'+MIN_OR_MAX+'_LST_in_cold_window/'+REGION+'/'+ICLIMP+'_with_trend_ANN_'+REGION+'.png}')
                f.write('\\includegraphics[trim=0 25 0 15,clip,width=0.49\\textwidth]{/scratch/vportge/plots/GHCNDEX/'+REGION+'/'+IGHCNDEX+'_time_series_GHCNDEX_with_trend_annually_'+REGION+'.png}')
                f.write('\\vskip\\baselineskip\n')
        ...
        with open('python_indices_2x3_subfigures'+MIN_OR_MAX+'_'+REGION+'.txt', 'w') as f:
            for INAME in python_indices:
                f.write('\n\n\n')
                f.write('\\begin{figure}\n')
                f.write('\\includegraphics[trim=0 25 0 15,clip,width=0.49\\textwidth]{/scratch/vportge/plots/Python_Indices/'+MIN_OR_MAX+'_LST_in_cold_window/'+REGION+'/'+INAME+'_with_trend_ANN_'+REGION+'.png}')
                f.write('\\includegraphics[trim=0 25 0 15,clip,width=0.49\\textwidth]{/scratch/vportge/plots/GHCNDEX/'+REGION+'/'+INAME+'_time_series_GHCNDEX_with_trend_annually_'+REGION+'.png}')
                f.write('\\vskip\\baselineskip\n')
